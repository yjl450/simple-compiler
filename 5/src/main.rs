...
fn compile_expr(
    ...
    match e {
        ...
        Expr::Boolean(true) => instrs.push(Instr::IMov(Val::Reg(Reg::RAX), Val::Imm(7))),
        Expr::Boolean(false) => instrs.push(Instr::IMov(Val::Reg(Reg::RAX), Val::Imm(3))),
        Expr::Input => {
            if fdef {
                match fn_name {
        ...
        Expr::Let(bindings, body) => {
            ...
            for bind in bindings {
                compile_expr(
                    &bind.1, instrs, &new_env, new_sp, count, exit_label, fn_name, fn_count, fdef,
                    false,
                );
                instrs.push(Instr::IMov(
                    Val::RegOffset(Reg::RBP, new_sp),
                    Val::Reg(Reg::RAX),
                ));
                new_env = new_env.update(bind.0.clone(), new_sp);
                new_sp += 1;
            }
            compile_expr(
                body, instrs, &new_env, new_sp, count, exit_label, fn_name, fn_count, fdef, tr,
            );
        }
        Expr::UnOp(op, e) => {
            compile_expr(
        ...
        Expr::Call(f, e) => {
            ...
            for arg in e.iter() {
                compile_expr(
                    arg, instrs, env, new_sp, count, exit_label, fn_name, fn_count, fdef, false,
                );
                instrs.push(Instr::IMov(
                    Val::RegOffset(Reg::RBP, new_sp),
                    Val::Reg(Reg::RAX),
            ...
            if tr && (Some(f.clone()) == *fn_name) {
                for i in 0..n {
                    instrs.push(Instr::IMov(
                        Val::Reg(Reg::RAX),
                        Val::RegOffset(Reg::RBP, new_sp - n + i),
                    ));
                    instrs.push(Instr::IMov(
                ...
                for i in 0..n {
                    instrs.push(Instr::IMov(
                        Val::Reg(Reg::RAX),
                        Val::RegOffset(Reg::RBP, new_sp - i - 1),
                    ));
        ...
        Expr::Vec(es) => {
            for (i, e) in es.iter().enumerate() {
                ...
                instrs.push(Instr::IMov(
                    Val::RegOffset(Reg::RBP, sp + (i as i32)),
                    Val::Reg(Reg::RAX),
                ));
            }
            instrs.push(Instr::IMov(Val::Reg(Reg::RAX), Val::Imm(es.len() as i64)));

            instrs.push(Instr::IMov(Val::RegOffset(Reg::R11, 0), Val::Reg(Reg::RAX)));
            ...
            for i in 0..es.len() {
                ...
                instrs.push(Instr::IMov(
                    Val::RegOffset(Reg::R11, 1 + i as i32),
                    Val::Reg(Reg::RAX),
                ));
            }
            instrs.push(Instr::IMov(Val::Reg(Reg::RAX), Val::Reg(Reg::R11)));
        ...
        Expr::VecGet(e, ind) => {
            compile_expr(
                ind, instrs, env, sp, count, exit_label, fn_name, fn_count, fdef, false,
            );
            instrs.extend(rax_check_num_decode(&Op2::Plus));
            instrs.extend(vec![
                Instr::ISar(Val::Reg(Reg::RAX), Val::Imm(1)),
                Instr::ICmp(Val::Reg(Reg::RAX), Val::Imm(0)),
                Instr::IMov(Val::Reg(Reg::RDI), Val::Error(Err::IndexOutOfBounds)),
                Instr::IJl("label_error".to_string()),
                Instr::IMov(Val::RegOffset(Reg::RBP, sp), Val::Reg(Reg::RAX)),
            ]);
            compile_expr(
                e, instrs, env, sp + 1, count, exit_label, fn_name, fn_count, fdef, false,
            );
            instrs.extend(vec![
                Instr::IMov(Val::Reg(Reg::RDX), Val::Reg(Reg::RAX)),
                Instr::IAnd(Val::Reg(Reg::RDX), Val::Imm(7)),
                Instr::ICmp(Val::Reg(Reg::RDX), Val::Imm(1)),
                Instr::IMov(Val::Reg(Reg::RDI), Val::Error(Err::GetNonVec)),
                Instr::IJne("label_error".to_string()),
                Instr::IMov(Val::Reg(Reg::RCX), Val::RegOffset(Reg::RBP, sp)),
                Instr::ISub(Val::Reg(Reg::RAX), Val::Imm(1)),
                Instr::IMov(Val::Reg(Reg::RDX), Val::RegOffset(Reg::RAX, 0)),
                Instr::ICmp(Val::Reg(Reg::RCX), Val::Reg(Reg::RDX)),
                Instr::IMov(Val::Reg(Reg::RDI), Val::Error(Err::IndexOutOfBounds)),
                Instr::IJge("label_error".to_string()),
                Instr::IAdd(Val::Reg(Reg::RCX), Val::Imm(1)),
                Instr::IMul(Val::Reg(Reg::RCX), Val::Imm(8)),
                Instr::IAdd(Val::Reg(Reg::RAX), Val::Reg(Reg::RCX)),
                Instr::IMov(Val::Reg(Reg::RAX), Val::RegOffset(Reg::RAX, 0)),
            ])
        }
    }
}
...
fn expr_vars(e: &Expr) -> i32 {
    match e {
        ...
        Expr::VecGet(e, eind) => max(expr_vars(e), 1+expr_vars(eind)),
    }
}
