...
pub fn val_to_asm(v: &Val) -> String {
    match v {
        ...
        Val::Imm(i) => format!("{}", i),
        Val::RegOffset(r, i) => match r {
            Reg::R11 => format!("[r11 + {}]", 8 * i),
            _ => format!("[{} - 8*({})]", reg_to_asm(&r), i),
        },
        Val::True => format!("7"),
        Val::False => format!("3"),
        Val::Error(e) => match e {
            ...
            Err::IndexOutOfBounds => format!("778899"),
            Err::GetNonVec => format!("123456"),
        },
    }
}
...
pub fn check_keyword(id: &str) -> bool {
    let keywords = [
        ...
        "*", "<", ">", "<=", ">=", "=", "true", "false", "input", "print", "vec", "vec-get",
    ];
...
pub fn parse_expr(s: &Sexp) -> Expr {
    match s {
        ...
        Sexp::List(vec) => match &vec[..] {
            ...
            [Sexp::Atom(S(op)), e, ind] if op == "vec-get" => {
                Expr::VecGet(Box::new(parse_expr(e)), Box::new(parse_expr(ind)))
            }
            [Sexp::Atom(S(op)), e1, e2] => match op.as_str() {
                ...
                _ => panic!("Invalid Sexp List Op2 {}", op),
            },
        ...
        [Sexp::Atom(S(op)), Sexp::List(es), body] if op == "fun" => {
            ...
            let args: Vec<String> = params
                ...
                .map(|e| match e {
                    Sexp::Atom(S(id)) => {
                        if check_keyword(id) {
                            panic!("Invalid argument name {id} in function definition")
                        }
                        id.to_string()}
